...
const isDarkCookie = useCookie("dark", {
    maxAge: 60 * 60 * 24 * 365,
})

isDarkCookie.value = isDarkCookie.value ?? String(usePreferredDark().value)

const isDark = ref(Boolean(isDarkCookie.value))

const toggleDark = () => {
    const cookie = useCookie("dark")
    const current = Boolean(cookie.value)
...
<template>
    ...
    <button @click="toggleDark" type="button"
        ...
        <svg v-if="!isDark" class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
